	...
	private  void deleteSetOfKeys(Set<String> listOfKeys, Pipeline deleteDataPipeline, Long campaignId) {
		...
		for (String key : listOfKeys) {
			if (key.startsWith(Constants.GENDER_KEY_PREFIX)) {
				deleteDataPipeline.srem(key, Constants.CAMPAIGN_REDIS_KEY+campaignId.toString());
				...
				minAgeValue =  getMatchingResult(key, Constants.REGEX_TO_GET_MIN_AGE).replaceAll(Constants.AGE_KEY_PREFIX, "");
				...
				if(key.indexOf(Constants.GENDER_KEY_PREFIX)!=-1) {
					/*********************** START- PROCESSING FOR THE KEY WHERE WE ARE GETTING THE KEY FOR AGE AND GENDER COMBINATION **********/
					 genderValue = getMatchingResult(key, Constants.REGEX_TO_GET_GENDER_FROM_GROUP);
					 if (!(GenericValidator.isBlankOrNull(minAgeValue)) && !(GenericValidator.isBlankOrNull(maxAgeValue))) {
					 for (int i = minAge; i <= maxAge; i++)
							deleteDataPipeline.srem(Constants.AGE_KEY_PREFIX + i+Constants.AGE_GENDER_SEPARATOR+genderValue,
									Constants.CAMPAIGN_REDIS_KEY+campaignId.toString());
						deleteDataPipeline.srem(Constants.AUDIENCE_KEY_PREFIX+campaignId.toString(), key);
					 }
					 else
							logger.error(key + " is giving false Age values....");
					
				}/*********************** END- PROCESSING FOR THE KEY WHERE WE ARE GETTING THE KEY FOR AGE AND GENDER COMBINATION **********/ 
				else {		
					/*********************** START- PROCESSING FOR THE KEY WHERE WE ARE GETTING THE KEY FOR AGE ONLY **********/
				if (!(GenericValidator.isBlankOrNull(minAgeValue)) && !(GenericValidator.isBlankOrNull(maxAgeValue))) {
					for (int i = minAge; i <= maxAge; i++)
				...
				}/*********************** END - PROCESSING FOR THE KEY WHERE WE ARE GETTING THE KEY FOR AGE ONLY **********/
				else
					logger.error(key + " is giving false Age values....");
				}/********** ENDS ELSE CONDITION WHERE WE HAVE ONLY AGE *********************/
			}
	...
	private  void setAudienceDetails(List<Row> rowList, Jedis jedis, Pipeline dataPipeline, HashMap<Long, Long> audienceRuleIdCampaignIdMap) {
		for (Row row : rowList) {
			...
			if (!(audienceMap.isEmpty())) {
				...
				/****************** NEW CODE ********************************/
				/************** CHECKING WHETHER WE HAVE ALL THE VALUES MIN AGE , MAX AGE & GENDER *****************************/
				if(!(GenericValidator.isBlankOrNull(minAgeString))&& !(GenericValidator.isBlankOrNull(maxAgeString))&&!(GenericValidator.isBlankOrNull(gender))) {
					for (int i = minAge; i <= maxAge; i++) {
						if(!(GenericValidator.isBlankOrNull(Constants.AGE_KEY_PREFIX + i))
								&&(!GenericValidator.isBlankOrNull(campaignId.toString())))
						dataPipeline.sadd(Constants.AGE_KEY_PREFIX + i+Constants.AGE_GENDER_SEPARATOR+gender, Constants.CAMPAIGN_REDIS_KEY+campaignId.toString());
					}
					
					/************ START- CREATING REFERENCE FOR DELETING THE KEYS ******************************************/
					if(!(GenericValidator.isBlankOrNull(campaignId.toString()))&& 
							!(GenericValidator.isBlankOrNull(Constants.AGE_KEY_PREFIX.toString() + minAge + Constants.AGE_GENDER_SEPARATOR + maxAge)))
					dataPipeline.sadd(Constants.AUDIENCE_KEY_PREFIX+campaignId.toString(),
							(Constants.AGE_KEY_PREFIX.toString() + minAge + Constants.AGE_GENDER_SEPARATOR + maxAge+Constants.AGE_GENDER_SEPARATOR+gender));
					/************ END - CREATING REFERENCE FOR DELETING THE KEYS ******************************************/
				}/********* ENDS IF CONDITION OF CHECKING WHETHER WE HAVE ALL THE THREE VALUES I.E. GENDER ,MIN AGE & MAX AGE **************/ 
				/****************** END - NEW CODE ********************************/
				else if(!(GenericValidator.isBlankOrNull(minAgeString))
						&& !(GenericValidator.isBlankOrNull(maxAgeString))) {
					
					/**********************CHECKING IF MIN & MAX ATTRIBUTE IS BLANK OR NULL THEN ONLY WE WILL CRETAE AGE KEYS*****************************/
					/****************** ITERATING MIN_AGE TO MAX AGE TO GENERATE KEYS FOR EACH AGE***************/
					for (int i = minAge; i <= maxAge; i++) {
						if(!(GenericValidator.isBlankOrNull(Constants.AGE_KEY_PREFIX + i))
								&&(!GenericValidator.isBlankOrNull(campaignId.toString())))
						dataPipeline.sadd(Constants.AGE_KEY_PREFIX + i, Constants.CAMPAIGN_REDIS_KEY+campaignId.toString());
					}
					/********************** END ITERATION ********************************************************/
					/************ START- CREATING REFERENCE FOR DELETING THE KEYS ******************************************/
					if(!(GenericValidator.isBlankOrNull(campaignId.toString()))&& 
							!(GenericValidator.isBlankOrNull(Constants.AGE_KEY_PREFIX.toString() + minAge + Constants.AGE_GENDER_SEPARATOR + maxAge)))
					dataPipeline.sadd(Constants.AUDIENCE_KEY_PREFIX+campaignId.toString(),
							(Constants.AGE_KEY_PREFIX.toString() + minAge + Constants.AGE_GENDER_SEPARATOR + maxAge));
					/************ END - CREATING REFERENCE FOR DELETING THE KEYS ******************************************/
				} /************************ END CHECKING ONLY AGE IS AVALIBLE ***********************************/
				else if(!(GenericValidator.isBlankOrNull(gender)) &&
						!(GenericValidator.isBlankOrNull(campaignId.toString()))) {
					/******************* INSTARING GENDER KEY INTO PIPELINE ****************************/
				dataPipeline.sadd(gender, Constants.CAMPAIGN_REDIS_KEY+campaignId.toString());
				...
				/************ END - CREATING REFERENCE FOR DELETING THE KEYS ******************************************/
				}/********************* END- CHECKING ONLY GENDER IS AVALIBLE *********************************/ 
			} // ends if condition audience info is available or not
			else {
