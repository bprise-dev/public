...
Jedis MQTTRedisConnection;
	...
	public void update(List<Long> ids, Session session, Jedis jedis) {
		...
		for (Row row : rows) {
			...
			try {
				...
				logger.error("ERROR:: CREATING MQTT APPLIACTION METHOD THROWS AN ERROR--> "+e);
			}
		...
		try {
			...
			// TODO Auto-generated catch block
			logger.error("ERROR:: WHILE INSERTING A LAST BATCH CREATING MQTT APPLIACTION METHOD THROWS AN ERROR--> "+e);
		}
		listOfCassandraOutput.clear();
		}
	...
	private void createMqttApplication(List<Row> listOfCassandraOutput, Jedis jedis, int databaseIndex) throws MQTTKeyCreationFailedException {
